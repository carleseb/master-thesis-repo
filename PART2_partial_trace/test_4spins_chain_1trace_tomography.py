...
trace_dm_tracker =  np.zeros((number_iterations, dimensions_expect_tracker)) #this one keeps the trace of the full dm
# it has to give 1 to all entries if the initial state and the evolution (unitary) give pure final state
...
"""
expect_base0_tracker = np.zeros((number_iterations, dimensions_expect_tracker))
base0 = basis(2,0)
base0_dm = base0*base0.dag()
amplitude_tracker_base0 = np.zeros((number_iterations, dimensions_expect_tracker))
...
for l in range(number_iterations):
    ...
    for k in range(dimensions_expect_tracker):
        ...
        # now we want to trace out the information of spins 2, 3 and 4
        finaldm.dims =  [[2, 2, 2, 2], [2, 2, 2, 2]]
        dm1 = finaldm.ptrace([0]) # We keep subspace 0
        trace_reduced_tracker[l,k] = (dm1*dm1).tr()
        
        # we get the expectation value of singlet in spin 1
        expect_base0_tracker[l,k] = (dm1*base0_dm).tr()
        expect_plus_tracker[l,k] = (dm1*plus_dm).tr()
    ...
    time_step = 1/sample_rate
    sig_fft_base0 = sp.fftpack.fft(expect_base0_tracker[l,:]) # we get the fft of the signal
    sig_fft_plus = sp.fftpack.fft(expect_plus_tracker[l,:])
    amplitude_base0 = np.abs(sig_fft_base0)
    amplitude_plus = np.abs(sig_fft_plus)
    sample_freq_base0 = sp.fftpack.fftfreq(expect_base0_tracker[l,:].size, d = time_step)
    sample_freq_plus = sp.fftpack.fftfreq(expect_plus_tracker[l,:].size, d = time_step)
    amplitude_base0[0] = 0 # we set the zero frequency amplitude
    amplitude_plus[0] = 0
    amplitude_tracker_base0[l,:] = amplitude_base0
    amplitude_tracker_plus[l,:] = amplitude_plus

print('done')

# we plot the oscillations of tr(rho2) of the reduced system
x = t
y = np.linspace(J12zero-2*J34ini, J12zero-2*J34fin, number_iterations)
plt.pcolormesh(x, y, trace_reduced_tracker)
plt.title('Oscillations of the trace of the reduced density matrix squared') #spin 1
plt.xlabel('time ($t_0$)')
plt.ylabel('$J_{12} - J_{34}$ ($E_0$)')
plt.colorbar()
plt.show()

# the fourier of the reduced osicllations
x = sample_freq_trace
y = np.linspace(J12zero-2*J34ini, J12zero-2*J34fin, number_iterations)
...
x = t
y = np.linspace(J12zero-2*J34ini, J12zero-2*J34fin, number_iterations)
plt.pcolormesh(x, y, expect_base0_tracker)
plt.title('Oscillations of the reduced system (proba 0)') #spin 1
plt.xlabel('time ($t_0$)')
plt.ylabel('$J_{12} - J_{34}$ ($E_0$)')
plt.colorbar()
plt.show()
...
x = sample_freq_base0
y = np.linspace(J12zero-2*J34ini, J12zero-2*J34fin, number_iterations)
plt.pcolormesh(x[:80], y, amplitude_tracker_base0[:,:80])
plt.title('Fourier transform of the oscillations of the reduced 0 state')
plt.xlabel('frequency ($f_0$)')
plt.ylabel('$J_{12} - J_{34}$ ($E_0$)')
plt.colorbar()
plt.show()
...
plt.title('Oscillations of the reduced system (proba plus)') #spin 1
plt.xlabel('time ($t_0$)')
plt.ylabel('$J_{12} - J_{34}$ ($E_0$)')
plt.colorbar()
plt.show()
...
# HERE WE START THE SECOND PART OF THE PROTOCOL (NOT DONE FOR SINGLE SPIN READOUT YET)
