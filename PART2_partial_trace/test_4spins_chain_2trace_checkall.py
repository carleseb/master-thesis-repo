...
"""
The aim of this script is to check if all the time evolution approaches
(DM solver unitary by stephan, tomography) yield the same.
This will be done with the first step of the 4spins chain method.

Seems that solver preserves pureness but not tomography
dimensions_expect_tracker set to 100

DM solver unitary does not allow us to get the state at any timestep
"""
...
dimensions_expect_tracker = 1000 # number of time steps 1000
...
density_matrices = np.zeros((number_iterations, dimensions_expect_tracker, 3, 3), dtype = complex)
density_matrices_squared = np.zeros((number_iterations, dimensions_expect_tracker, 3, 3), dtype = complex)
...
#    ket0 = basis(3, 1)
    ...
    for k in range(dimensions_expect_tracker):
        ...
        density_matrices_squared[n,k,:,:] = density_matrices[n,k,:,:]*density_matrices[n,k,:,:]
...
#print(density_matrices[0,-1,:,:], density_matrices_squared[0,-1,:,:])
print(np.trace(density_matrices[0,-1,:,:]), np.trace(density_matrices_squared[0,-1,:,:]))
#print(dens_mat, dens_mat*dens_mat)
print(np.trace(dens_mat), np.trace(dens_mat*dens_mat))
print(dens_mat.norm(), (dens_mat*dens_mat).norm())
...
expect_singlet_tracker = np.zeros((number_iterations, dimensions_expect_tracker), dtype = complex)
singlet = (tensor(basis(2,0), basis(2,1)) - tensor(basis(2,1), basis(2,0))).unit()
singlet_dm = singlet*singlet.dag()
expect_triplet_tracker = np.zeros((number_iterations, dimensions_expect_tracker), dtype = complex)
...
y = np.linspace(J12zero-2*J34ini, J12zero-2*J34fin, number_iterations)
#plt.pcolormesh(x, y, trace_dm_tracker)
plt.pcolormesh(x, y, np.real(expect_singlet_tracker))
...
y = np.linspace(J12zero-2*J34ini, J12zero-2*J34fin, number_iterations)
#plt.pcolormesh(x, y, trace_dm_tracker)
plt.pcolormesh(x, y, np.real(expect_triplet_tracker))
