...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  8 18:20:34 2023
...
state_ = tensor(Tminus, Tminus, S13T24)
state_t_ = basis_transformation(state_, trans_matrix).unit()
print(state_t_[200]) #1 at 200
ket1 = (-np.sqrt(24)*basis(7,1) + basis(7,4) + np.sqrt(35)*basis(7,5)).unit()
...
for J46 in values_J46:
    ...
    Jij_vector = np.array([0.01, 0, 0.01, 0, 0.01, 0, 0.01])
    Jij_ladder = np.array([1, 1.5, J46, 1.5, 1, 1.5])
    spins = len(Jij_vector) + 1
    H = hheis_general(Jij_vector, spins, B) + ladder_exchanges(Jij_ladder, spins)
