...
Jij_vector = np.array([1, 1, 1]) # in natural units
...
gs = ((np.sqrt(3)+1)*p1 - p2).unit() #factor obtained from Tzu-Kan's paper
print(gs) #same as groundstate
# If we want the ground state for the bc case
gs_bc = (p1-p2).unit()
...
for J23 in values_J23:
    # We first want to create the arbitrary Hamiltonian and print the matrix
    #Jij_vector = np.array([J12zero-J23, 0.6, J23])
    Jij_vector = np.array([1, J23, 1])
    spins = len(Jij_vector) + 1
    H = hheis_general(Jij_vector, spins, B) #+ chain_bc(1, spins)
