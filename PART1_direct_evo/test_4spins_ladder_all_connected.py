...
from hamiltonian import hheis_general, chain_bc, heisenberg_hamiltonian_4, ladder_exchanges, is_unitary
...
H = hheis_general(Jij_vector, spins, B) + ladder_exchanges(Jij_ladder, spins) + chain_bc(1, spins)
H.check_herm()
...
for J34 in values_J34:
    # We first want to create the arbitrary Hamiltonian and print the matrix
    Jij_vector = np.array([J12zero-J34, 1, J34])
    Jij_ladder = np.array([1, 1])
    spins = len(Jij_vector) + 1
...
plt.figure()
'''
plt.plot(J13zero-2*np.linspace(J24ini, J24fin, number_iterations), np.transpose(energy_tracker_)[0,:], label ='first triplet list (label 2)')
plt.plot(J13zero-2*np.linspace(J24ini, J24fin, number_iterations), np.transpose(energy_tracker_)[1,:], label ='second triplet list (label 3)')
plt.plot(J13zero-2*np.linspace(J24ini, J24fin, number_iterations), np.transpose(energy_tracker_)[2,:], label ='third triplet list (label 4)')
'''
