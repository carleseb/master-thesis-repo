...
for J46 in values_J46:
    ...
    Jij_ladder = np.array([J35zero - J46, J35zero - J46, 0, 0.5, J46, J46])
    ...
    #Jij_vector = np.array([0.5, 0, 0, 0, 0, 0, 0])
    #Jij_ladder = np.array([J35zero - J46, J46, J35zero - J46 , J46, J35zero - J46, J46])
    
    Jij_vector = np.array([J35zero - J46, 0, J35zero - J46, 0, J46, 0, J46])
    Jij_ladder = np.array([J35zero - J46, J35zero - J46, 0.7, 0.7, J46, J46])
...
#plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.concatenate((energy_tracker[:7,1], energy_tracker[7:,2])) , label ='$E2$')
#plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.concatenate((energy_tracker[:7,2], energy_tracker[7:,1])) , label ='$E3$')
...
"""
# a better plot for p1w1s2sep
plt.figure()
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[3,:] - np.transpose(energy_tracker)[0,:], label ='E4 - E1')
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[4,:] - np.transpose(energy_tracker)[0,:], label ='E5 - E1')
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[4,:] - np.transpose(energy_tracker)[3,:], label ='E5 - E4')
#plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[6,:] - np.transpose(energy_tracker)[2,:], label ='E7 - E3')
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[6,:] - np.concatenate((energy_tracker[:7,1], energy_tracker[7:,2])), label ='E7 - E2', linestyle='dashed')
plt.legend()
plt.axvline(x=0, color='k', linestyle='dotted')
plt.xlabel('$J_{p} - J_{mid}$ ($E_0$)')
plt.ylabel('energy ($E_0$)')
plt.title('Energy differences of eigenstates in the septuplet subspace')
plt.show()
"""

# a better plot for p1w2s1sep
plt.figure()
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[1,:] - np.transpose(energy_tracker)[0,:], label ='E2 - E1')
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[6,:] - np.transpose(energy_tracker)[5,:], label ='E7 - E6')
plt.plot(J35zero-2*np.linspace(J46ini, J46fin, number_iterations), np.transpose(energy_tracker)[2,:] - np.transpose(energy_tracker)[1,:], label ='E3 - E2')
...
plt.xlabel('$J_{35} - J_{46}$ ($E_0$)')
plt.ylabel('energy ($E_0$)')
plt.title('Energy differences of some states of the septuplet subspace')
plt.show()